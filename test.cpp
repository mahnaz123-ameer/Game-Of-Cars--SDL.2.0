 ...
 bool init()
{
	...
	{
		//Create window
		gWindow = SDL_CreateWindow( "Car_Racing_XX", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
		if( gWindow == NULL )
		{
...
bool loadMedia()
{
	...
	gmenuTexture=loadTexture("menuImage.bmp");
	ghighscoreTexture=loadTexture("highscore.bmp");
	ginstructionTexture=loadTexture("instructionImage.bmp");
	glevel2Texture=loadTexture("level2.bmp");
	glevel1Texture=loadTexture("level1.bmp");
	gloadingTexture=loadTexture("loading.bmp");
	glevel3Texture=loadTexture("level3.bmp");
	gGameTexture=loadTexture("game.bmp");
	//Load car
	gCARTexture[0]=loadTexture( "carred.bmp" );
	gCARTexture[1]=loadTexture( "carblue1.bmp" );
	gCARTexture[2]=loadTexture( "carwhite.bmp" );
	gCARTexture[3]=loadTexture( "carblue2.bmp" );
	gCARTexture[4]=loadTexture( "carash.bmp" );
    //load other things	
	glifeTexture=loadTexture("life.bmp");
	glife1Texture=loadTexture("life1.bmp");
	gboyTexture=loadTexture("man.bmp");
	gcoinTexture=loadTexture("coin.bmp");
	gcoin1Texture=loadTexture("coin1.bmp");
	Sans = TTF_OpenFont("me.ttf",20);
	return success;
}
//update high score
	                ...
	                if(gameState==2){
						...
						while(i<5)
							{	fscanf(fp, "%s", s);
								...
								SDL_Texture * Message = SDL_CreateTextureFromSurface(gRenderer, surfaceMessage);
								SDL_Rect Message_rect = {150,(i*70)+240, 40, 40};
								SDL_RenderCopy(gRenderer, Message, NULL, &Message_rect);
								i++;
							}
							fclose(fp);
